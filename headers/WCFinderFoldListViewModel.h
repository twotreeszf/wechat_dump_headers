//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

#import "WCFinderDataItemExt-Protocol.h"

@class NSData, NSMutableArray, NSMutableSet, NSString;
@protocol WCFinderFoldListViewModelDelegate;

@interface WCFinderFoldListViewModel : NSObject <WCFinderDataItemExt>
{
    id <WCFinderFoldListViewModelDelegate> _delegate;
    NSMutableArray *_dataItems;
    NSMutableSet *_tidContainSet;
    NSData *_lastBuff;
    NSString *_targetUsername;
    NSString *_feedID;
    NSString *_nonceID;
    unsigned long long _feedScreenStyle;
}

- (void).cxx_destruct;
@property(nonatomic) unsigned long long feedScreenStyle; // @synthesize feedScreenStyle=_feedScreenStyle;
@property(copy, nonatomic) NSString *nonceID; // @synthesize nonceID=_nonceID;
@property(copy, nonatomic) NSString *feedID; // @synthesize feedID=_feedID;
@property(copy, nonatomic) NSString *targetUsername; // @synthesize targetUsername=_targetUsername;
@property(retain, nonatomic) NSData *lastBuff; // @synthesize lastBuff=_lastBuff;
@property(retain, nonatomic) NSMutableSet *tidContainSet; // @synthesize tidContainSet=_tidContainSet;
@property(retain, nonatomic) NSMutableArray *dataItems; // @synthesize dataItems=_dataItems;
@property(nonatomic) __weak id <WCFinderFoldListViewModelDelegate> delegate; // @synthesize delegate=_delegate;
- (void)onFinderDataItemDelete:(id)arg1;
- (long long)getIndexOfTid:(id)arg1;
- (id)contentVMWithTid:(id)arg1;
- (void)stopPreloadVideoItem:(id)arg1;
- (void)preloadVideoItems:(id)arg1;
- (long long)contentIndexOfTid:(id)arg1;
- (void)fetchMoreLikeList;
- (_Bool)deleteDataItemWithIndex:(unsigned long long)arg1 tid:(id)arg2;
- (id)transformDataItems:(id)arg1;
- (void)dealloc;
- (id)initWithFriendUserName:(id)arg1 feedId:(id)arg2 nonceID:(id)arg3 localDataItem:(id)arg4 feedScreenStyle:(unsigned long long)arg5;
- (id)initWithFriendUserName:(id)arg1 feedId:(id)arg2 nonceID:(id)arg3 localDataItem:(id)arg4;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

