//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "MMUIViewController.h"

#import "EGORefreshTableHeaderDelegate-Protocol.h"
#import "MMScrollActionSheetDelegate-Protocol.h"
#import "MMSightCameraViewControllerDelegate-Protocol.h"
#import "UISearchBarDelegate-Protocol.h"
#import "UITableViewDataSource-Protocol.h"
#import "UITableViewDelegate-Protocol.h"
#import "WCActionSheetDelegate-Protocol.h"
#import "WCCommitViewResultDelegate-Protocol.h"
#import "WCDataSearchDelegate-Protocol.h"
#import "WCFinderCameraViewControllerDelegate-Protocol.h"
#import "WCFinderCommentDetailViewControllerDelegate-Protocol.h"
#import "WCFinderCommentIdentityViewControllerDelegate-Protocol.h"
#import "WCFinderContactExt-Protocol.h"
#import "WCFinderContentColumnTableViewCellDelegate-Protocol.h"
#import "WCFinderContentColumnViewControllerDelegate-Protocol.h"
#import "WCFinderContentTableViewCellDelegate-Protocol.h"
#import "WCFinderFavFoldTableViewCellDelegate-Protocol.h"
#import "WCFinderFeedBaseViewControllerProtocol-Protocol.h"
#import "WCFinderFeedCommentTableViewCellDelegate-Protocol.h"
#import "WCFinderFeedDescriptionTableViewCellDelegate-Protocol.h"
#import "WCFinderFeedFriendsLikeListViewControllerDelegate-Protocol.h"
#import "WCFinderFeedRecommendVMDelegate-Protocol.h"
#import "WCFinderFeedRecommendVMExt-Protocol.h"
#import "WCFinderFriendLikeTableViewCellDelegate-Protocol.h"
#import "WCFinderFriendLiveSectionTableViewCellDelegate-Protocol.h"
#import "WCFinderInduceCommentTableViewCellDelegate-Protocol.h"
#import "WCFinderLiveNoticeTableViewCellDelegate-Protocol.h"
#import "WCFinderMentionExt-Protocol.h"
#import "WCFinderMoreHotContentBottomViewDelegate-Protocol.h"
#import "WCFinderMoreHotContentTableViewCellDelegate-Protocol.h"
#import "WCFinderMoreSimilarViewControllerDelegate-Protocol.h"
#import "WCFinderMultiMediaTableViewCellDelegate-Protocol.h"
#import "WCFinderNewsTopicTableViewCellDelegate-Protocol.h"
#import "WCFinderPopularGuideTableViewCellDelegate-Protocol.h"
#import "WCFinderPostViewControllerDelegate-Protocol.h"
#import "WCFinderPostingCommentExt-Protocol.h"
#import "WCFinderPostingCommentFailTableViewCellDelegate-Protocol.h"
#import "WCFinderPublishHeadTableViewCellProtocol-Protocol.h"
#import "WCFinderRecommendTabsViewDelegate-Protocol.h"
#import "WCFinderRedDotExt-Protocol.h"
#import "WCFinderRedDotNotifyExt-Protocol.h"
#import "WCFinderRefreshTableFooterViewDelegate-Protocol.h"
#import "WCFinderTLFeedPrivateTableViewCellDelegate-Protocol.h"
#import "WCFinderTLFriendLikeFeedFoldTableViewCellDelegate-Protocol.h"
#import "WCFinderTabViewControllerProtocol-Protocol.h"
#import "WCFinderTimelinePrivateMsgTableViewCellDelegate-Protocol.h"
#import "WCFinderTipsViewControllerDelegate-Protocol.h"
#import "WCFinderToolbarTableViewCellDelegate-Protocol.h"
#import "WCFinderUserPrepareCgiDelegate-Protocol.h"
#import "WCFinderVideoPlayerExt-Protocol.h"
#import "WCGeneralListMonitorExt-Protocol.h"
#import "WCGeneralMonitorDelegate-Protocol.h"
#import "WCSearchDelegate-Protocol.h"

@class MMScrollViewProxy, MMUISearchBar, NSIndexPath, NSMutableDictionary, NSString, UIImageView, UILabel, UIMonitorTableView, UITapGestureRecognizer, UIView, UIViewController, WCActionSheet, WCFinderFeedContentVM, WCFinderFeedDescriptionTableViewCell, WCFinderFeedRecommendVM, WCFinderFriendLiveSectionTableViewCell, WCFinderPullDownRefreshHeaderView, WCFinderRefreshTableFooterView;
@protocol WCFinderCommentBaseViewProtocol, WCFinderFeedRecommendViewControllerDelegate;

@interface WCFinderFeedRecommendViewController : MMUIViewController <UITableViewDelegate, UITableViewDataSource, WCFinderFeedRecommendVMExt, WCFinderContactExt, WCFinderMentionExt, WCFinderRefreshTableFooterViewDelegate, EGORefreshTableHeaderDelegate, WCSearchDelegate, WCDataSearchDelegate, WCCommitViewResultDelegate, WCFinderRedDotNotifyExt, WCFinderPublishHeadTableViewCellProtocol, MMSightCameraViewControllerDelegate, WCFinderCameraViewControllerDelegate, WCFinderMultiMediaTableViewCellDelegate, WCFinderToolbarTableViewCellDelegate, WCFinderContentTableViewCellDelegate, WCFinderFeedCommentTableViewCellDelegate, WCActionSheetDelegate, MMScrollActionSheetDelegate, UISearchBarDelegate, WCGeneralListMonitorExt, WCFinderPostingCommentExt, WCFinderPostingCommentFailTableViewCellDelegate, WCFinderPostViewControllerDelegate, WCFinderFeedDescriptionTableViewCellDelegate, WCFinderCommentDetailViewControllerDelegate, WCFinderCommentIdentityViewControllerDelegate, WCFinderVideoPlayerExt, WCFinderRedDotExt, WCFinderTipsViewControllerDelegate, WCFinderUserPrepareCgiDelegate, WCFinderFriendLikeTableViewCellDelegate, WCFinderRecommendTabsViewDelegate, WCFinderFeedRecommendVMDelegate, WCFinderFeedBaseViewControllerProtocol, WCFinderMoreHotContentBottomViewDelegate, WCFinderMoreHotContentTableViewCellDelegate, WCFinderTLFriendLikeFeedFoldTableViewCellDelegate, WCFinderTimelinePrivateMsgTableViewCellDelegate, WCFinderFeedFriendsLikeListViewControllerDelegate, WCFinderPopularGuideTableViewCellDelegate, WCFinderTLFeedPrivateTableViewCellDelegate, WCFinderFavFoldTableViewCellDelegate, WCFinderInduceCommentTableViewCellDelegate, WCGeneralMonitorDelegate, WCFinderContentColumnTableViewCellDelegate, WCFinderMoreSimilarViewControllerDelegate, WCFinderContentColumnViewControllerDelegate, WCFinderFriendLiveSectionTableViewCellDelegate, WCFinderNewsTopicTableViewCellDelegate, WCFinderLiveNoticeTableViewCellDelegate, WCFinderTabViewControllerProtocol>
{
    _Bool _hasReportCreateEvent;
    _Bool _isSilencePlay;
    _Bool _isDisableVideoAutoPlay;
    _Bool _footerNoMoreData;
    _Bool _needsResumeTimelineVideo;
    _Bool _isEnableMarkRead;
    _Bool _isViewWillAppeaer;
    _Bool _hasAppear;
    _Bool _hideSearchBar;
    id <WCFinderFeedRecommendViewControllerDelegate> _delegate;
    UIMonitorTableView *_tableView;
    WCFinderFeedRecommendVM *_feedVM;
    NSString *_latestAutoplayVideoTid;
    WCFinderRefreshTableFooterView *_footerView;
    NSIndexPath *_currentForceIndexPath;
    WCFinderFeedContentVM *_currentShareContentVM;
    WCFinderPullDownRefreshHeaderView *_refreshHeaderView;
    MMUISearchBar *_searchBar;
    UIView *_searchContainer;
    NSMutableDictionary *_exposureDict;
    UILabel *_stateTipsLabel;
    UITapGestureRecognizer *_stateRetryGesture;
    UIImageView *_retryImageView;
    double _latestContentViewY;
    UIView *_moreFeedActionView;
    WCFinderFeedDescriptionTableViewCell *_moreFeedCell;
    UIViewController<WCFinderCommentBaseViewProtocol> *_lastCommentVC;
    NSString *_currentVideoTid;
    NSString *_currentForceTid;
    double _tableViewStartDragOffsetY;
    long long _lastDisappearSecond;
    WCActionSheet *_forwardActionSheet;
    MMScrollViewProxy *_scrollProxy;
    WCFinderFriendLiveSectionTableViewCell *_livingCell;
    UIView *_fakeNavigationBar;
    UIView *_maskView;
}

+ (_Bool)hookScrollViewDidScroll;
+ (_Bool)hookScrollViewDidEndDecelerating;
+ (_Bool)hookScrollViewDidEndDraggingWillDecelerate;
+ (_Bool)hookReloadDataWrap;
+ (_Bool)hookViewDidBePoped;
+ (void)load;
- (void).cxx_destruct;
@property(retain, nonatomic) UIView *maskView; // @synthesize maskView=_maskView;
@property(retain, nonatomic) UIView *fakeNavigationBar; // @synthesize fakeNavigationBar=_fakeNavigationBar;
@property(nonatomic) _Bool hideSearchBar; // @synthesize hideSearchBar=_hideSearchBar;
@property(retain, nonatomic) WCFinderFriendLiveSectionTableViewCell *livingCell; // @synthesize livingCell=_livingCell;
@property(retain, nonatomic) MMScrollViewProxy *scrollProxy; // @synthesize scrollProxy=_scrollProxy;
@property(retain, nonatomic) WCActionSheet *forwardActionSheet; // @synthesize forwardActionSheet=_forwardActionSheet;
@property(nonatomic) long long lastDisappearSecond; // @synthesize lastDisappearSecond=_lastDisappearSecond;
@property(nonatomic) _Bool hasAppear; // @synthesize hasAppear=_hasAppear;
@property(nonatomic) double tableViewStartDragOffsetY; // @synthesize tableViewStartDragOffsetY=_tableViewStartDragOffsetY;
@property(copy, nonatomic) NSString *currentForceTid; // @synthesize currentForceTid=_currentForceTid;
@property(nonatomic) _Bool isViewWillAppeaer; // @synthesize isViewWillAppeaer=_isViewWillAppeaer;
@property(nonatomic) _Bool isEnableMarkRead; // @synthesize isEnableMarkRead=_isEnableMarkRead;
@property(copy, nonatomic) NSString *currentVideoTid; // @synthesize currentVideoTid=_currentVideoTid;
@property(nonatomic) _Bool needsResumeTimelineVideo; // @synthesize needsResumeTimelineVideo=_needsResumeTimelineVideo;
@property(retain, nonatomic) UIViewController<WCFinderCommentBaseViewProtocol> *lastCommentVC; // @synthesize lastCommentVC=_lastCommentVC;
@property(retain, nonatomic) WCFinderFeedDescriptionTableViewCell *moreFeedCell; // @synthesize moreFeedCell=_moreFeedCell;
@property(retain, nonatomic) UIView *moreFeedActionView; // @synthesize moreFeedActionView=_moreFeedActionView;
@property(nonatomic) _Bool footerNoMoreData; // @synthesize footerNoMoreData=_footerNoMoreData;
@property(nonatomic) double latestContentViewY; // @synthesize latestContentViewY=_latestContentViewY;
@property(retain, nonatomic) UIImageView *retryImageView; // @synthesize retryImageView=_retryImageView;
@property(retain, nonatomic) UITapGestureRecognizer *stateRetryGesture; // @synthesize stateRetryGesture=_stateRetryGesture;
@property(retain, nonatomic) UILabel *stateTipsLabel; // @synthesize stateTipsLabel=_stateTipsLabel;
@property(retain, nonatomic) NSMutableDictionary *exposureDict; // @synthesize exposureDict=_exposureDict;
@property(retain, nonatomic) UIView *searchContainer; // @synthesize searchContainer=_searchContainer;
@property(retain, nonatomic) MMUISearchBar *searchBar; // @synthesize searchBar=_searchBar;
@property(nonatomic) _Bool isDisableVideoAutoPlay; // @synthesize isDisableVideoAutoPlay=_isDisableVideoAutoPlay;
@property(retain, nonatomic) WCFinderPullDownRefreshHeaderView *refreshHeaderView; // @synthesize refreshHeaderView=_refreshHeaderView;
@property(retain, nonatomic) WCFinderFeedContentVM *currentShareContentVM; // @synthesize currentShareContentVM=_currentShareContentVM;
@property(retain, nonatomic) NSIndexPath *currentForceIndexPath; // @synthesize currentForceIndexPath=_currentForceIndexPath;
@property(nonatomic) _Bool isSilencePlay; // @synthesize isSilencePlay=_isSilencePlay;
@property(retain, nonatomic) WCFinderRefreshTableFooterView *footerView; // @synthesize footerView=_footerView;
@property(retain, nonatomic) NSString *latestAutoplayVideoTid; // @synthesize latestAutoplayVideoTid=_latestAutoplayVideoTid;
@property(retain, nonatomic) WCFinderFeedRecommendVM *feedVM; // @synthesize feedVM=_feedVM;
@property(retain, nonatomic) UIMonitorTableView *tableView; // @synthesize tableView=_tableView;
@property(nonatomic) _Bool hasReportCreateEvent; // @synthesize hasReportCreateEvent=_hasReportCreateEvent;
@property(nonatomic) __weak id <WCFinderFeedRecommendViewControllerDelegate> delegate; // @synthesize delegate=_delegate;
- (void)liveNoticeUserame:(id)arg1 clickLiveNoticeInfo:(id)arg2;
- (void)liveNoticeTableViewCell:(id)arg1 updateLiveNoticeInfo:(id)arg2;
- (void)newsTopicTableViewCellDidClick:(id)arg1 withTopic:(id)arg2;
- (void)onFeedVMLiveDataRefreshData;
- (void)onFeedVMLiveDataNoMore;
- (void)onFeedVMLiveDataChanged;
- (void)exposeLiveSectionWithContentVM:(id)arg1 endExpose:(_Bool)arg2;
- (void)finderLiveSectionTableViewCell:(id)arg1 endExposeLiveContentVM:(id)arg2;
- (void)finderLiveSectionTableViewCell:(id)arg1 didExposeLiveContentVM:(id)arg2;
- (void)finderLiveSectionTableViewCellFetchMoreLiveList:(id)arg1;
- (void)onDidExposeLiveAtIndex:(id)arg1 inLives:(id)arg2;
- (void)onDidSelectLiveAtIndex:(id)arg1 inLives:(id)arg2;
- (void)onItemsExposed:(id)arg1 withOldItems:(id)arg2;
- (void)contentColumnViewControllerClickMoreFeedAction:(id)arg1 lastBuffer:(id)arg2 dataItemArray:(id)arg3;
- (void)finderMoreSimilarViewControllerClickMoreFeedAction:(id)arg1;
- (void)contentColumnTableViewCell:(id)arg1 layoutContentVM:(id)arg2 clickCoverContact:(id)arg3 contentVM:(id)arg4;
- (void)contentColumnTableViewCell:(id)arg1 clickRefreshForContentVM:(id)arg2;
- (void)displayContentColumnListVCWithVM:(id)arg1 targetVM:(id)arg2;
- (void)contentColumnTableViewCell:(id)arg1 clickAllSectionWithlayoutContentVM:(id)arg2 isClickTipsAction:(_Bool)arg3;
- (void)contentColumnTableViewCell:(id)arg1 layoutContentVM:(id)arg2 didSelectContentVM:(id)arg3;
- (void)onClickInduceCommnetView:(id)arg1 forFeedVM:(id)arg2 identifyType:(unsigned long long)arg3 commentPostScene:(long long)arg4;
- (void)finderFavFoldTableViewCellDidSelected:(id)arg1 layoutContentVM:(id)arg2;
- (void)finderTLFeedPrivateCell:(id)arg1 stateChange:(_Bool)arg2;
- (void)popularGuideTableViewCell:(id)arg1 clickRefreshForContentVM:(id)arg2;
- (void)popularGuideTableViewCell:(id)arg1 layoutContentVM:(id)arg2 didSelectContentVM:(id)arg3;
- (void)popularGuideTableViewCell:(id)arg1 clickAllSectionWithlayoutContentVM:(id)arg2 isClickTipsAction:(_Bool)arg3;
- (void)onClickFinderPrivateMsgNotify;
- (void)WCFinderTLFriendLikeFeedFoldTableViewCellDidClickLabelButtonWithOriginDataItem:(id)arg1 targetIndex:(unsigned long long)arg2 isMoreIndex:(_Bool)arg3;
- (void)onMoreHotContentCellClick;
- (void)onMoreHotBottomContentClick;
- (id)getCurMaskView;
- (id)getCurrentDataItemWithSessionBuffer;
- (id)getCurViewModelLastBuff;
- (unsigned long long)tabViewControllerGetCurTableViewVisibleTopIndex;
- (void)tabViewControllerAutoRefreshTrigerByRedDot;
- (void)refreshHeadOnScrolltoTop;
- (unsigned long long)getCurTableViewVisibleBottomIndex;
- (_Bool)tabViewControllerGetSelectState;
- (void)handleHotContentExposeLogic;
- (void)tabViewControllerStopCurrentPlayVideo;
- (void)handleDataReportLogicViewHidden;
- (void)triggerDeselectLogic;
- (void)triggerSelectLogic;
- (void)tabViewControllerSelectionDidChange:(_Bool)arg1;
- (unsigned long long)feedViewerScene;
- (unsigned long long)feedViewControllerScene;
- (_Bool)onFeedRecommendGetHotTabNeedTopRefresh;
- (id)onFeedRecommendGetHotTabLastBuff;
- (void)onFeedRecommendVMShouldReloadAtIndexPath:(id)arg1;
- (_Bool)WCFinderFeedRecommendVMIsSelected;
- (void)finderRedDotDataChangedWithPathKey:(id)arg1;
- (_Bool)triggerRefreshLogic:(unsigned int)arg1 toTab:(unsigned int)arg2;
- (void)handleDataReportLogicViewShow;
- (void)updateExposureStatusOfContentVM:(id)arg1;
- (void)pourStateIntoPool:(id)arg1;
- (void)exposeMarkRead:(id)arg1;
- (void)handleMoreFeedCellResetLogic;
- (void)handleFeedExposeLogic;
- (void)clickRecommentUrl:(id)arg1;
- (void)updateCommentDetailVCPushState:(id)arg1;
- (void)clickCommentNickname:(id)arg1 username:(id)arg2 contact:(id)arg3 isWeChatFriend:(_Bool)arg4 dataItem:(id)arg5;
- (void)onFinderVideoPlayerStartPlay;
- (void)showDataItemWasDeletedTips;
- (void)showDetailDataItemVC:(id)arg1;
- (void)finderPostingCommentFailTableViewCellDidClick:(id)arg1;
- (void)onFinderDataItemPostingCommentFailBySpecialError:(id)arg1 errorCode:(int)arg2 errorMsg:(id)arg3;
- (void)deleteFeedWithContentVM:(id)arg1;
- (void)reportForwordWith:(id)arg1;
- (void)scrollActionSheetDidTapTransparent:(id)arg1;
- (void)scrollActionSheetDidCancel:(id)arg1;
- (void)didClickToolbarDeleteAction:(id)arg1;
- (void)scrollActionSheet:(id)arg1 didSelecteItem:(id)arg2;
- (void)onClickNotificationMsgBtn;
- (void)onDoneForwardForForwardView:(id)arg1;
- (void)onCancelForwardForForwardView:(id)arg1;
- (_Bool)searchBarShouldBeginEditing:(id)arg1;
- (void)finderFeedDescriptionCellDidClick:(id)arg1 isFloat:(_Bool)arg2;
- (void)onClickCommentUserName:(id)arg1 contentVM:(id)arg2;
- (void)finderCommentCell:(id)arg1 didClickCommentLikeWithcomment:(id)arg2 contentVM:(id)arg3;
- (void)onRefreshMediaUI:(id)arg1 needAnimation:(_Bool)arg2;
- (void)onRefreshCommentUI:(id)arg1 needAnimation:(_Bool)arg2;
- (void)onClickCommentAction:(id)arg1 contentVM:(id)arg2;
- (void)onClickContentMentionAction:(id)arg1 nickname:(id)arg2 dataItem:(id)arg3;
- (void)onClickContentTopicAction:(id)arg1 dataItem:(id)arg2;
- (void)onClickContentExtReadingAction:(id)arg1;
- (void)onContentTableViewCell:(id)arg1 longPressContentAction:(id)arg2 rect:(struct CGRect)arg3;
- (void)onClickContentReadMoreAction:(id)arg1 isExpand:(_Bool)arg2;
- (void)onRefreshContentCellUI:(id)arg1;
- (void)onClickEventEntry:(id)arg1;
- (void)onClickLongEntry:(id)arg1;
- (void)onClickContentUsernameAction:(id)arg1;
- (void)clickContentPOIAction:(id)arg1;
- (void)onRefreshToolbarUI:(id)arg1;
- (void)actionSheet:(id)arg1 clickedButtonAtIndex:(long long)arg2;
- (void)onClickToolbarShareAction:(id)arg1;
- (void)showCollectAnimtion:(id)arg1;
- (void)onClickToolbarCollectionAction:(id)arg1;
- (void)onClickToolbarCancelUploading:(id)arg1;
- (void)onClickToolbarRetryUploading:(id)arg1;
- (void)showCommentListWithContentVM:(id)arg1 showEnterKeyBoard:(_Bool)arg2 refCommentID:(unsigned long long)arg3;
- (void)onClickViewAllCommentAction:(id)arg1 isActiveInput:(_Bool)arg2;
- (void)onClickToolbarLikeAction:(id)arg1 isPrivateLike:(_Bool)arg2;
- (void)doShowInduceCommentForCurrentPhoto:(id)arg1;
- (void)tryShowInduceCommentForCurrentPhoto:(id)arg1 preContentVM:(id)arg2;
- (void)showLongVideoBubble:(id)arg1;
- (void)reloadInduceCommentCell:(id)arg1;
- (void)onShowLongVideoBubble:(id)arg1;
- (void)onVideoPlayWithReport:(id)arg1 mediaWrap:(id)arg2 currentIndex:(unsigned long long)arg3 forContentVM:(id)arg4;
- (_Bool)onMultiMediaCellIsInHotSpots:(id)arg1;
- (_Bool)finderMulitMediaTableViewCellCanProgressGestureWithTid:(id)arg1;
- (void)onHeaderFollowBtnClickAction:(id)arg1 followButton:(id)arg2;
- (void)onHeaderMoreClickAction:(id)arg1;
- (void)onFeedVideoStopPlay:(id)arg1 tid:(id)arg2;
- (void)contentMediaDidEndPlay:(unsigned long long)arg1 contentVM:(id)arg2 finderPlayerReport:(id)arg3;
- (void)retryLoadingVideoResource:(_Bool)arg1;
- (void)photoContentPageTurning:(id)arg1 currentPage:(unsigned long long)arg2 lastPage:(unsigned long long)arg3;
- (void)showSpamTipsWithString:(id)arg1;
- (void)likeFeedContentVMAction:(id)arg1 isPrivateLike:(_Bool)arg2;
- (void)didFeedDoubleLikeAction:(id)arg1;
- (void)onFeedCellDidClickLiveNowView:(id)arg1 contentVM:(id)arg2;
- (void)onFeedCellDidClickHeaderView:(id)arg1 contentVM:(id)arg2;
- (void)didSelectVideoMediaMuteChanged:(_Bool)arg1;
- (void)onLiveFeedDidTap:(id)arg1;
- (void)onClickLikedListContactWithWith:(id)arg1 LikeInfo:(id)arg2 ObjectID:(id)arg3;
- (void)onClickLikedListClickMoreActionWith:(id)arg1 ObjectId:(id)arg2;
- (void)finderFriendLikeTableViewCellClickLikeAction:(id)arg1 contentVM:(id)arg2 isNormalLiked:(_Bool)arg3;
- (void)clickRecommendUrl:(id)arg1;
- (void)onRefreshFriendUI:(id)arg1;
- (void)onClickFriendGroupListDetail:(id)arg1;
- (void)onClickFriendLikeListDetail:(id)arg1;
- (void)onClickSearch;
- (void)scrollViewWillEndDragging:(id)arg1 withVelocity:(struct CGPoint)arg2 targetContentOffset:(inout struct CGPoint *)arg3;
- (void)scrollViewWillBeginDragging:(id)arg1;
- (void)tableView:(id)arg1 willDisplayFooterView:(id)arg2 forSection:(long long)arg3;
- (void)tableView:(id)arg1 willDisplayCell:(id)arg2 forRowAtIndexPath:(id)arg3;
- (void)exposureTime:(id)arg1 index:(long long)arg2;
- (void)tableView:(id)arg1 didEndDisplayingCell:(id)arg2 forRowAtIndexPath:(id)arg3;
- (double)tableView:(id)arg1 heightForFooterInSection:(long long)arg2;
- (double)tableView:(id)arg1 heightForHeaderInSection:(long long)arg2;
- (id)tableView:(id)arg1 viewForHeaderInSection:(long long)arg2;
- (id)tableView:(id)arg1 viewForFooterInSection:(long long)arg2;
- (void)reloadDataWrap;
- (id)getListView;
- (id)specialParamsForFeedId:(id)arg1;
- (id)feedIdByIndex:(id)arg1;
- (id)feedCellViewParamModelWithContentVM:(id)arg1;
- (double)tableView:(id)arg1 heightForRowAtIndexPath:(id)arg2;
- (long long)numberOfSectionsInTableView:(id)arg1;
- (long long)tableView:(id)arg1 numberOfRowsInSection:(long long)arg2;
- (id)tableView:(id)arg1 cellForRowAtIndexPath:(id)arg2;
- (void)scrollViewDidScroll:(id)arg1;
- (void)scrollViewDidEndDecelerating:(id)arg1;
- (void)scrollViewDidEndDragging:(id)arg1 willDecelerate:(_Bool)arg2;
- (void)scrollViewDidScrollToTop:(id)arg1;
- (_Bool)scrollViewShouldScrollToTop:(id)arg1;
- (_Bool)egoRefreshTableHeaderDataSourceIsLoading:(id)arg1;
- (void)stopRefreshHeaderLoadingCompletion:(CDUnknownBlockType)arg1 delay:(double)arg2 refreshState:(unsigned long long)arg3;
- (void)triggerActiveRefreshAction;
- (void)triggerAutoRefreshActionByClickStatusBar:(_Bool)arg1;
- (void)startTriggerRefreshHeaderLoadingMode:(unsigned long long)arg1;
- (void)egoRefreshTableHeaderDidTriggerRefresh:(id)arg1;
- (void)resetMoreFeedCell;
- (void)onFeedAllDataItemsNeedClean;
- (void)processLastVideoHunger;
- (void)resetFooterViewNormal;
- (void)onDataEmptyState:(_Bool)arg1;
- (void)showMoreContentView;
- (void)onRefreshTableViewResetContentInset;
- (void)onFeedVMCommentFailNeedReload;
- (void)onLayoutContentVMDidUpdate:(id)arg1 oldLayoutVM:(id)arg2 atIndexPath:(id)arg3;
- (void)onFeedVMDataItemMediaResourceIsDeleted;
- (void)onFeedLastestVideoMuteNeedChange;
- (void)onFeedPrivateMsgTipsUpdate;
- (void)onFeedVMGetNewMsg;
- (void)onFeedVMDataTextDidShowFullText:(id)arg1;
- (void)onFeedVMDataWillRemoved:(id)arg1;
- (void)onFeedVMDataChangedAtIndexPath:(id)arg1 contentVM:(id)arg2 animated:(_Bool)arg3;
- (void)onFeedVMDataChangedAtIndexPath:(id)arg1 contentVM:(id)arg2;
- (void)onFeedVMDataFootRefreshError;
- (void)onFeedVMDataFootRefreshNoMore;
- (void)onFeedVMDataPullRefreshError:(unsigned long long)arg1;
- (void)onFeedVMDataPullRefreshNoMore;
- (void)onFeedVMDataPullRefreshStopLoading;
- (void)onFeedServerFetchHistorySuccess:(_Bool)arg1;
- (void)onFeedServerFetchSuccess;
- (void)onFeedVMDataFetchSuccess;
- (void)onFeedVMDataChangedByPublish:(id)arg1;
- (void)onFeedVMDataDeleteWithIndexSet:(id)arg1;
- (void)onFeedVMDataAppend:(id)arg1;
- (void)onFeedVMInsertContentVM:(id)arg1 section:(long long)arg2;
- (void)onFeedVMDataAppendFromIndex:(long long)arg1 toIndex:(long long)arg2;
- (void)onFeedRevertPlayerState;
- (void)onFeedVMDataChanged;
- (void)onFeedVMWillChangeOriginDataIndex:(_Bool)arg1;
- (void)onFeedVMDataLocalEmpty;
- (_Bool)isEnableTriggerRefreshAtScroll:(id)arg1;
- (void)finderFooterDidTriggerRefresh:(id)arg1;
- (void)didClickFooterRefreshRetry:(id)arg1;
- (void)fetchNextPage;
- (void)preloadVideoItemsWithCurrentFeedNextIndex:(unsigned long long)arg1;
- (void)preloadVideoItems;
- (void)currentForceIndexPathWillChanged;
- (void)reportColumnContentVM:(id)arg1 index:(long long)arg2;
- (void)reportStatsWithoutTidImmediately:(id)arg1;
- (void)reportFullStatsExceptTidImmediately:(id)arg1;
- (void)reportFullStatsImmediately;
- (void)checkCurrentForceIndexPath;
- (void)autoplayVideoProcess;
- (id)getVisibleVideoIndexPathArray:(id)arg1;
- (void)clickNotifyTipsAction;
- (void)scrollEndProcess;
- (void)throttleScrollProcess;
- (void)onTimelineViewEnterBackground:(id)arg1;
- (void)onTimelineViewEnterForeground:(id)arg1;
- (void)removeObserverForForegroundNotification;
- (void)addObserverForForegroundNotification;
- (void)reloadRowAtIndexPath:(id)arg1 contentVM:(id)arg2 animation:(long long)arg3;
- (void)scrollTimelineToTopWithAnimation:(_Bool)arg1;
- (void)scrollTimelineToTop;
- (void)updateCurrentVideoThumb;
- (void)resetCurrentVideoThumb;
- (void)needsUpdateCurrentVideo:(_Bool)arg1;
- (void)markVisibleContentCardWillDisplay;
- (void)markVisibleFoldFeedStartDisPlay;
- (void)markVisibleFoldFeedEndDisplay;
- (void)stopPreloadVisibleVideo;
- (void)stopVisibleVideo;
- (void)stopCurrentMedia;
- (void)resetRetryState;
- (void)stateRetryGestureAction:(id)arg1;
- (void)dealloc;
- (void)willDisappear;
- (struct CGRect)getCurSearchBarFrame;
- (void)exposeCurrentDisplayFeed;
- (void)viewDidAppear:(_Bool)arg1;
- (void)initSearchBar;
- (void)setUpFooterView;
- (void)setUpRefreshHeaderView;
- (void)setUpTableView;
- (double)topMargin;
- (double)visibleSearchContainerHeight;
- (void)initSubview;
- (_Bool)useTransparentNavibar;
- (void)viewDidLoad;
- (id)getGeneralListMonitorMgr;
- (_Bool)isOpenGeneralListSessionStatSwitch;
- (void)checkScrollEnd:(id)arg1;
- (void)tsScrollViewDidScroll:(id)arg1;
- (void)tsScrollViewDidEndDragging:(id)arg1 willDecelerate:(_Bool)arg2;
- (void)tsScrollViewDidEndDecelerating:(id)arg1;
- (void)onFinderTabDisappear:(unsigned int)arg1;
- (void)onFinderTabAppear:(unsigned int)arg1;
- (void)tsReloadDataWrap;
- (void)tsViewDidBePoped:(_Bool)arg1;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

